...
\noindent Although CFLs are closed under homomorphisms allowing unified lexing and parsing, most languages explicitly define a separate lexer, which we can use to abstract over named identifiers. Given an invalid string, the tool will first map the string, then generate edits over abstracted space, and finally map the successful repairs back to the original space, as shown below:
...
This coarsening reduces the number of possible corrections. We rerank valid completions using a probability metric as described in \S\ref{sec:adaptive}.
...
Tidyparse uses a CFG to parse the CFG, so it can provide assistance while the user is designing the CFG. For example, if the CFG does not parse, will suggest possible fixes.% In the future, we intend to use this functionality to perform example-based codesign and grammar induction.
...
Users can interactively build up a complex expression by placing the caret over a nonterminal they wish to expand, then pressing \keys{\ctrl + \SPACE}:

...
Our architecture accepts a grammar and a string, which can contain any arbitrary characters. Given an input string, we first attempt to parse using Valiant's algorithm~\cite{valiant1979completeness}. If the string is syntactically correct, we return the parse forest, otherwise, we return the partial parse forest and set of edits that would make the string syntactically correct. This is always guaranteed to exist, as the trivial solution is to delete the entire string and replace it with the shorest string in the language.

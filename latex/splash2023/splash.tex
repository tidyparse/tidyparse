...
%\begin{itemize}
%  \item CFL Parsing
%  \item Notation (where to put?)
%  \item Valiant's Matrix Multiplication Algorithm
%  \item Levenshtein Edits (insert, delete, substitution)
%  \item Problem definition (independent section?)
%\end{itemize}

Three important questions arise when repairing any sort of program: (1) is the program broken in the first place? (2) if so, where are the errors located? (3) how should those locations then be altered? In the case of syntax correction, those questions are addressed in three related research areas, (1) parsing, (2) language equations and (3) repair. We survey each of those areas in turn.
...
General CFL parsing allows ambiguity (non-unique trees) and be formulated as a dynamic programming problem, as shown by Cocke-Younger-Kasami (CYK)~\cite{sakai1961syntax}, Earley~\cite{earley1970efficient} and others, whose parsers have roughly cubic time complexity with respect to the length of the input string.

As shown by Valiant~\cite{valiant1975general}, Lee~\cite{lee2002fast} and others, general CFL recognition is in some sense equivalent to binary matrix multiplication, another well-studied combinatorial problem in circuit lower bounds that is known to be subcubic. This realization unlocks the door to a wide range of complexity-theoretic and practical speedups to CFL recognition and fast general parsing algorithms.
...
Being an algebraic framework, language equations also lead to a kind of calculus, akin to that of Lebniz and Newton. First realized by Brzozowski~\cite{brzozowski1964derivatives, brzozowski1980equations} and Antimirov~\cite{antimirov1996partial}, one can also take the derivative of a language equation, yielding another equation, which can be interpreted as a kind of language quotient. This technique leads to a family of elegant algorithms for incremental parsing~\cite{might2011parsing, adams2016complexity} and automata minimization~\cite{brzozowski1962canonical}. In our particular setting, the Brzozowski derivative, or left quotient, corresponds to finding the suffixes that complete a known prefix, i.e., code completion.

In this paper, we restrict our attention to holes in context-free and weakly context-sensitive languages, coinciding with edit locations in the source code of computer programs. Although prior work has considered language equations for parsing~\cite{might2011parsing}, to our knowledge they have never previously been considered for the purpose of code completion or syntax error correction.
...
When a sentence is not syntactically valid, parsing becomes more challenging. Parsing with errors or \textit{error recovery} was first considered by Aho~\cite{aho1972minimum} and others. The problem is to find the minimum number of edits required to transform an arbitrary string into a string in the language. In practice one typically assumes a fixed distance metric with a small upper bound.

In finite languages, this amounts to spelling correction, which has existed for many decades. Schulz and Stoyan~\cite{schulz2002fast} introduce Levenshtein automata for spelling correction with a finite dictionary, which can be adapted to compute context-free language edit distance using a language intersection operator. Bar-Hillel~\cite{bar1961formal} has a construction that shows the intersection between a CFL and an NFA is still context-free.

Although CFLs are generally quite expressive, they are themselves not closed under intersection and have other practical limitations, i.e., are unable to capture many syntactic features of programming languages, e.g., indentation or variable binding. These limitations motivate us towards more expressive yet still efficiently parsable formalisms, such as weakly context-sensitive languages.

Okhotin~\cite{okhotin2010decision} extends the language equations to conjunctive language, which are CFLs closed under intersection.

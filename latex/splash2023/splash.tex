...
We seek to find the set of strings $S$ such that $\forall \tilde{\sigma}\in S, \Delta(\err{\sigma}, \tilde{\sigma}) \leq k$, where $k$ is the maximum number of edits greater than or equal to the language edit distance. We call this set the \textit{Levenshtein hypersphere} of $\err{\sigma}$ and denote it $\Delta_k(\err{\sigma})$. Since $1 \leq \Delta^*(\err{\sigma}, \ell) \leq k$, we have $1 \leq k$. We now consider an upper bound on $\Delta^*(\err{\sigma}, \ell)$, i.e., the greatest lower bound on $k$ such that $\Delta_k(\err{\sigma}) \cap \ell \neq \varnothing$.
...
\begin{lemma}
The complexity of exhaustively searching $A$ is upper bounded by $\mathcal{O}\left({{kn + n + 1} \choose k}(|\Sigma| + 1)^k\right)$.
\end{lemma}

\begin{proof}
We can overestimate the size of $A$ by considering the number of unique ways to insert, delete, or substitute $k$ terminals into a string $|\err{\sigma}|$ of size $n$. This can be overaproximated by interspersing $\varepsilon^k$ around every token, i.e., $\err{\sigma}'\coloneq \left(\varepsilon^k\err{\sigma}_i\right)_{i=1}^n\varepsilon^k$, where $|\err{\sigma}'| = kn + n + 1$, and only considering substitution. To admit deletion, we augment $\Sigma_\varepsilon \coloneq \Sigma \cup \{\varepsilon\}$, thus deletions and insertions may be considered as special cases of substitution. Thus, we have $kn + n + 1$ positions to insert $(|\Sigma| + 1)$ tokens, i.e., ${{kn + n + 1} \choose k}(|\Sigma| + 1)^k$ ways to edit $\err{\sigma}'$. This upper bound is not tight, because it double counts many identical edits w.r.t. $\err{\sigma}$. Nonetheless, it is sufficient to show $|A| < {{kn + n + 1} \choose k}(|\Sigma| + 1)^k$.
\end{proof}

In practice, this problem is ill-posed even when $k = \Delta^*(\err{\sigma}, \ell)$. For example, consider the language of ursine dietary preferences. Although $\err{\sigma}\coloneq$ ``Bears like to eat tomatoes'' is not in the language, e.g., $\tilde{\sigma}\coloneq$``Bears like to eat'' is valid $(\Delta^*=1)$, however there are many other variations with the same edit distance, e.g., or ``Bears like to eat \{\hlorange{berries}, \hlorange{honey}, \hlorange{fish}\}''. We may also wish to consider two or more edits, e.g., ``\{\hlgreen{Polar}, \hlgreen{Panda}\} bears like to eat \{\hlgreen{seals}, \hlgreen{bamboo}\}''. In general, there are many valid strings within a fixed Levenshtein distance of $\err{\sigma}$, and we seek to find among them, the set of strings that are both syntactically valid and semantically plausible within a short amount of time.
...
\section{Human Repair Benchmark}\label{sec:human}
...
ylabel={Precision@1},
symbolic x coords={1-edit, 2-edit, 3-edit},
xtick=data,
nodes near coords,
nodes near coords align={vertical},
ymin=0,ymax=1,
]
\addplot coordinates {(1-edit, 0.9) (2-edit, 0.857) (3-edit, 0.794)};
\addplot coordinates {(1-edit, 0.454) (2-edit, 0.162) (3-edit, 0.096)};
\addplot coordinates {(1-edit, 0.005) (2-edit, 0.004) (3-edit, 0.0)};
...
\addplot coordinates {(1-edit, 1585.465) (2-edit, 1953.56) (3-edit, 2744.887)};
\legend{Latency}
\end{axis}
\end{tikzpicture}
}
...
\section{Repair with an adaptive sampler}

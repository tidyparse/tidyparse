...
Syntax correction is one of the first problems that novice programmers encounter when learning to write code and a perennial issue in computer programming. Put simply, it is the problem of editing a syntactically incorrect program so that it parses. This problem is challenging not only because well-formed programs have many constraints, increasing the search complexity, but also because of the under-determined problem structure: repairs must align with the surrounding code context, and the correct repair is seldom unique, even assuming minimality.

Much prior work in syntax correction is a combination of two approaches: (1) \textit{pattern matching}, which uses hand-crafted rules to identify and correct common syntax errors, and (2) \textit{machine learning}, i.e., training a model to edit code using supervised or semi-supervised methods. The former approach is effective but requires language-specific rules, while the latter generates more natural edits, but requires training a language model on a large dataset, generalizes poorly under distributional shift, and is difficult to incorporate new constraints without retraining.

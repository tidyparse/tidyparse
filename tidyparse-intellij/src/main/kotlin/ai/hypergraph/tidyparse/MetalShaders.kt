...
// Expected output:
//    Reduced L-NFA(len=36, rad=5, states=222) from 1086 to 1086 arcs in 13.645834ms
//    Completed prefix scan in: 18.792ms
//    Encoded instance in 403048 bytes / 12665988 / 77.790458ms
//    Constructed 222x222x257 parse chart in 2.418583 ms
//    Fixpoint escape at round: 20/222, total=686247, size: 25331976
//    GPU prefix sum in 11.474084 ms
//    Built backpointers in 24.871625 ms
//    Sampled words in 18.541 ms
//    Round trip repair: 245.947792ms
//    GPU repairs: 250 valid / 250 unique / 1000 total
...
fun main() {
  ...
  fun List<List<List<Int>>>.parallelPrefixScan(): Pair<IntArray, IntArray> = measureTimedValue {
  ...
  measureTimeMillis {
    ...
    val (valid, invalid) = samples
      .joinToString(" ") { if (it + 0 == 0) "0" else cfg.tmLst[it.toInt() - 1] }
...
inline void sampleTopDown(
  ...
  const device int*     bpStorage,
    ...
    constexpr int MAX_STACK = 1024; int stack[MAX_STACK]; int top = 0; int wordLen = 0; stack[top++] = startDPIdx;
...
kernel void sample_words(
  ...
  const device int*     startIndices    [[buffer(4)]], // each entry is a dpIndex
    ...
    sampleTopDown(dp_in, bpCount, bpOffset, bpStorage, startIdx, rngState, localWord, maxWordLen);

    for (int i=0; i<maxWordLen; i++) sampledWords[int(tid)*maxWordLen + i] = localWord[i];
}

kernel void prefix_sum_p1(
    const device int*   inBuf   [[buffer(0)]],
...
kernel void prefix_sum_p2(
    ...
    const device int*    blkSum  [[buffer(1)]],
...
@_cdecl("repairPipeline") public func repairPipeline(
    ...
    memcpy(dp_out, sampledWordsBuf.contents(), Int(maxSamples * maxWordLen))
}
...
private func buildBackpointers(
    ...
    let countSize  = totalCells * stride
    ...
    parallelPrefixSumGPU(bpCountBuf, bpOffsetBuf, totalCells)
    ...
    let offCountPtr = UnsafeBufferPointer(start: bpOffsetBuf.contents().assumingMemoryBound(to: Int32.self), count: totalCells)
    let bpOffSum = offCountPtr.reduce(0) { $0 + Int64($1) }
    print("bpOffsetBufSize: \(bpOffSum)")
...
func parallelPrefixSumGPU(_ countBuf: MTLBuffer, _ offsetBuf: MTLBuffer, _ totalCells: Int) {
    let t0 = DispatchTime.now()
...
func runKernel(_ ps: String, _ tds: Int, _ tpg: Int = 1, body: (MTLComputeCommandEncoder) -> Void) {
    let commandBuffer = queue.makeCommandBuffer()!, enc = commandBuffer.makeComputeCommandEncoder()!
    enc.setComputePipelineState(pipelineStates[ps]!)
    body(enc)
    enc.dispatchThreads(
        MTLSize(width: tds, height: 1, depth: 1), 
...
private var device: MTLDevice!, queue: MTLCommandQueue!, numNonterminals: Int = 0, startIdx: Int = 0,
            pipelineStates: [String: MTLComputePipelineState] = [:]

@_cdecl("setup") public func setup(numNts: CInt, startIndex: CInt, mtlSrc: UnsafePointer<CChar>?) {
    device = MTLCreateSystemDefaultDevice()!
    queue = device.makeCommandQueue()!

    let metalSrc = mtlSrc != nil ? String(cString: mtlSrc!) : ""
    numNonterminals = Int(numNts)
    startIdx = Int(startIndex)
    let library = try! device.makeLibrary(source: metalSrc, options: nil)
    pipelineStates = Dictionary(uniqueKeysWithValues: [
         "cfl_mul_upper",
         "bp_count",
         "bp_write",
         "sample_words",
         "prefix_sum_p1",
         "prefix_sum_p2"
      ].map { ($0, try! device.makeComputePipelineState(function: library.makeFunction(name: $0)!)) }
    )
}"""
  ...
  private fun genFlattenedBinaryProds(cfg: CFG): String {
    ...
    return """constant int vilen=${grammarFlattened.size}; constant int volen=${grammarOffsets.size};
              ...
              constant int constant vidx_offsets[]={${grammarOffsets.joinToString(",")}};""".trimIndent()
  }

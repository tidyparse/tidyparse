...
function browserClangdAssets() {
  ...
  const worker = path.join(
    repository,
    "build/js/packages/Tidyparse-tidyparse-cpp/kotlin/Tidyparse-tidyparse-cpp.js"
  );
  const resourceDirectories = [
    path.join(repository, "build/js/packages/Tidyparse-tidyparse-cpp-test/kotlin"),
    path.join(repository, "tidyparse-cpp/src/jsMain/resources")
  ];
  ...
  if (sidecars.length === 0) {
    return {
      error: `No pinned native validator exists for ${browser.manifest.artifactVersion}; ` +
        "run :tidyparse-cpp:refreshClangdResources"
    };
  }
  const failures = [];

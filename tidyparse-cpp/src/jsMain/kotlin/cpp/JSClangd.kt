...
private val CPP_COI_SERVICE_WORKER_VERSION = "6-" + CPP_CLANGD_ARTIFACT_VERSION
    ...
    if (!invalidCacheMode) {
      val response = js("""(request) => {
        ...
        if (clangdWorkerRequest) {
          target = new URL("tidyparse-cpp.js?cpp-worker-bundle=clangd", url);
...
suspend fun ensureCppCrossOriginIsolation(): String? {
  ...
  return try {
    val scriptUrl = js("(base, version) => new URL('tidyparse-cpp.js?cpp-coi=' + version, base).href")(
      document.baseURI,
